...
int main()
{
	...
	std::cout << "call cca init" << std::endl;

	status = ecall_cca_init(global_eid, &target_ptr);
	...
	std::cout << "cca init completed" << std::endl;

	void *page_start = (void*)(target_ptr & ~(page_size - 1));
	int ret = mprotect(page_start, page_size, PROT_NONE);

	std::cout << "mprotect result: " << ret << std::endl;

	status = ecall_cca(global_eid);
